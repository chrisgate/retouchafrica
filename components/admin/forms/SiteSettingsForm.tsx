...
import { Field, TextArea, FormError } from "@/components/admin/forms/fields";

export function SiteSettingsForm({ settings: s }: { settings: SiteSettings }) {
  ...
  return (
    <form action={formAction} className="mt-6 flex max-w-2xl flex-col gap-8">
      ...
      {state?.success && <p className="border border-green-600/30 bg-green-600/5 px-3 py-2 text-sm text-green-700">Settings saved.</p>}
      ...
      <button
        type="submit"
        disabled={pending}
        className="w-fit bg-ink px-6 py-3 text-xs font-semibold uppercase tracking-[0.15em] text-paper hover:bg-ink/80 disabled:opacity-50"
      >
